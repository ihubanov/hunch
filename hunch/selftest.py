...
_COUNTRIES = ["Germany", "Spain", "Italy", "France", "Portugal", "Belgium", "Austria", "Poland", "Greece", "Sweden",
              "Norway", "Denmark", "Finland", "Ireland", "Netherlands", "Czechia", "Hungary", "Romania", "Bulgaria", "Croatia"]
...
async def run(names: list[str]) -> int:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for name in names:
            ...
            if r.status_code != 200:
                print(f"FAIL constraint probe: HTTP {r.status_code} {r.text[:200]}")
                failures += 1
                continue
            d = r.json()
            content = (d["choices"][0]["message"].get("content") or "").strip()
            if content not in ("A", "B"):
                print(f"FAIL structured_outputs NOT enforced (got {content!r}); answers would be unconstrained")
                failures += 1
                continue
            print("ok   structured_outputs enforced")
            if not (d["choices"][0].get("logprobs") or {}).get("content"):
                print("FAIL no logprobs returned")
                failures += 1
                continue
